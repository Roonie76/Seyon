...
function CustomSelect({ id, label, value, options, onChange, icon, placeholder }: CustomSelectProps) {
  ...
  return (
    <div ref={containerRef} className="relative flex flex-col gap-1.5 w-full md:w-auto min-w-[160px]">
      {label && (
        <label htmlFor={id} className="text-[11px] font-bold text-zinc-400 uppercase tracking-wider select-none">
          {label}
        </label>
      )}
      <button
          ...
          {options.map((opt) => (
            ...
            >
              ...
              <span className="flex items-center gap-1.5 shrink-0">
                {opt.count !== undefined && (
                  <span className="text-[11px] font-semibold text-zinc-500 bg-zinc-100 rounded-md px-1.5 py-0.5">
                    {opt.count}
                  </span>
                )}
        ...
        <div className="hidden md:flex flex-col w-full">
          ...
          <div className="flex items-center gap-4">
            ...
            >
              ...
              <span>Filters</span>
              {activeFilterCount > 0 && (
                <span className="flex items-center justify-center h-5 min-w-5 px-1.5 text-[11px] font-bold text-black bg-amber-500 rounded-full animate-fade-in">
                  {activeFilterCount}
                </span>
              )}
